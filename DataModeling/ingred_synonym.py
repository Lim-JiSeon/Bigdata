...
df = pd.read_excel('C:/Users/ksc/Desktop/0. 원본/조리법_필터링_종합2.xlsx')
...
### 재료사전 불러오기
df2 = pd.read_excel('C:/Users/ksc/Desktop/재료사전.xlsx')
...
# 중복된 데이터의 빈도수 합치기
df3 = df2.groupby('재료').sum().reset_index()
# df2 = df2.groupby('재료', as_index=False)['재료'].sum()

df3.sort_values(by=['빈도수'],ascending=False,inplace=True,ignore_index=True)
# print(df2)
df3 = df3[(df3['빈도수']>=250)]

for i in range(len(df3)):
    try:
      ingredient = df3.at[i, '재료']
      # print(ingredient)
      result = model.wv.most_similar(ingredient)
      similar_words = [word[0] for word in result if word[1] >= 0.8]
      for j in range(i+1, len(df3)):
        if df3['재료'][j] in similar_words:
          print('--> {0} replaced with {1}'.format(df3['재료'][j], df3['재료'][i]))
          df3['재료'][j] = df3['재료'][i]
...
df3 = df3.groupby('재료').sum().reset_index()
# df2 = df2.groupby('재료', as_index=False)['재료'].sum()

df3 = df3.sort_values(by=['빈도수'],ascending=False)
print(df3)

# 두 데이터프레임을 합병시켜 속성 컬럼 표시
ingred_rep = pd.merge(df3, df2[['재료', '속성']], on='재료', how='left')
ingred_rep = ingred_rep.drop_duplicates(subset='재료')


# 데이터프레임 엑셀로 저장
ingred_rep.to_excel('C:/Users/ksc/Desktop/재료사전_유의어ver5.xlsx')
